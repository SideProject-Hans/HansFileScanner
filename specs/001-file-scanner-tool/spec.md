...
### User Story 2 - 以不同模式檢視檔案並搜尋 (Priority: P2)

身為使用者，我想要以不同模式（樹結構、文檔、圖檔、影片、音訊清單）檢視掃描結果，並透過搜尋功能快速定位特定檔案，以便更聚焦地找到需要的檔案。

**Why this priority**: 這是提升使用者體驗的重要功能，讓使用者可以快速篩選和定位特定類型的檔案。搜尋功能進一步強化了篩選能力，讓使用者在大量檔案中能更精準地找到目標，是日常使用中非常實用的功能。

**Independent Test**: 在掃描完成後，切換不同顯示模式、使用下拉選單篩選檔案類型、並使用搜尋框輸入關鍵字，驗證顯示結果是否正確。
...
8. **Given** 使用者在任一顯示模式，**When** 使用者在搜尋框輸入關鍵字，**Then** 系統即時篩選並顯示檔案名稱包含該關鍵字的結果
9. **Given** 使用者已選擇檔案類型篩選，**When** 使用者在搜尋框輸入關鍵字，**Then** 系統在已篩選的結果中進一步搜尋符合的檔案
10. **Given** 使用者輸入搜尋關鍵字，**When** 沒有任何檔案符合搜尋條件，**Then** 系統顯示「找不到符合條件的檔案」訊息
11. **Given** 使用者已輸入搜尋關鍵字，**When** 使用者點擊清除按鈕或清空搜尋框，**Then** 系統恢復顯示當前模式與篩選條件下的所有檔案
...
- 當掃描過程中使用者關閉應用程式，如何處理？→ **設計決策：不提供取消功能，掃描一旦開始必須完成。若使用者強制關閉應用程式，下次開啟時從初始狀態開始。**
- 當檔案正在被其他程式使用時，如何處理刪除或複製操作？→ **設計決策：顯示錯誤訊息說明檔案被鎖定，跳過該檔案繼續處理其他檔案，操作完成後顯示失敗清單供使用者檢視。**
- 當目標複製資料夾與來源資料夾相同時，如何處理？→ **設計決策：偵測到相同資料夾時顯示錯誤提示「目標資料夾不能與來源資料夾相同」，要求使用者重新選擇不同的目標資料夾。**
...
#### 搜尋功能
- **FR-022**: 系統必須在所有顯示模式下提供搜尋框，讓使用者輸入關鍵字搜尋檔案
- **FR-023**: 搜尋功能必須支援即時搜尋（即時篩選），在使用者輸入時動態更新結果
- **FR-024**: 搜尋功能必須支援檔案名稱的部分比對（包含關鍵字即符合）
- **FR-025**: 搜尋功能必須不區分大小寫
- **FR-026**: 搜尋功能必須與檔案類型篩選功能協同運作，在已篩選的結果中進行搜尋
- **FR-027**: 系統必須提供清除搜尋的功能（清除按鈕或清空搜尋框）
...
- **SearchFilter（搜尋篩選）**: 代表當前的搜尋條件，包含關鍵字、是否區分大小寫等設定
...
- **SC-008**: 搜尋結果在使用者停止輸入後 300 毫秒內更新顯示
...
### Session 2025-12-08
- Q: 這個跨平台桌面應用程式應該使用哪種技術框架來建構？ → A: Tauri + React/Vue（Rust 後端 + Web 前端）
- Q: 當使用者在掃描進行中想要取消掃描時，系統應該如何處理？ → A: 不提供取消功能，掃描一旦開始必須完成
- Q: 當檔案正在被其他程式使用時，如何處理刪除或複製操作？ → A: 顯示錯誤訊息說明檔案被鎖定，跳過該檔案並繼續處理其他檔案，最後顯示失敗清單
- Q: 當目標複製資料夾與來源資料夾相同時，如何處理？ → A: 偵測到相同資料夾時顯示錯誤提示，要求使用者選擇不同的目標資料夾
- Q: 在 Tauri 框架的前端部分，應該使用哪個前端框架？ → A: React（成熟穩定，元件庫豐富，社群支援廣泛）

---

## Technical Stack

- **框架**: Tauri 2.x（Rust 後端 + Web 前端）
- **前端**: React 18+（搭配 TypeScript）
- **UI 元件庫**: 待確認（建議選項：Ant Design、Shadcn/ui、Radix UI）
- **後端**: Rust（透過 Tauri 提供原生檔案系統操作）
- **優勢**: 相較 Electron 更輕量、更快、記憶體佔用更低
